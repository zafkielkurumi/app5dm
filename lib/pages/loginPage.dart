  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: ChangeNotifierProvider<UserModel>(
          create: (_) => UserModel(),
          child: Theme(
            data: Theme.of(context).copyWith(splashFactory: NoSplashFactory()),
            child: ListView(
              children: <Widget>[
                Stack(
                  children: <Widget>[
                                  ...
                                  SizedBox(
                                      ...
                                      child: Selector<UserModel, UserModel>(
                                        ...
                                        builder: (_, userModel, child) {
                                          return RaisedButton(
                                            onPressed: () async {
                                              if(userModel.viewState != ViewState.pending) {
                                                var res = await userModel.login(pwd: pwdController.value.text, userName: userNameController.value.text);
                                                if (res == true) {
                                            ...
                                            child: userModel.viewState == ViewState.pending ? Text('登录中'): Text('登录'),
                                          );
                                        },
                                      )),
                                  SizedBox(
